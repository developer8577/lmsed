          ...
          <tbody className="text-gray-700">
            {enrolledCourses.map((course, index) => (
              <tr key={index} className="border-b border-gray-500/20">
                ...
                <td className="px-4 py-3 max-sm:hidden">
                  {progressArray[index] && `${progressArray[index].totalLectures}`} <span className="text-gray-600">Lectures</span>
                </td>

                <td className="px-4 py-3 max-sm:text-right">
                  <button className="bg-blue-200/40 text-blue-700 px-3 p-1 rounded-md font-medium" onClick={() => navigate('/player/' + course._id)}>
